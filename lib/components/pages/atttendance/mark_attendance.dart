...
class _MarkAttendancePageState extends State<MarkAttendancePage>
  ...
  bool _isShiftOver() {
    if (_state.loginTime == null) return false;
    return DateTime.now().isAfter(_getShiftEndTime());
  }
  ...
  Future<void> _completeLogin() async {
    // Guard: if already logged in, don't re-run complete login.
    if (_state.isLoggedIn) {
  ...
  void _updateState(AppState newState) {
    if (mounted && !_isDisposed && _state != newState) {
      ...
      // Debounce state saves to prevent rapid consecutive saves
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    switch (state) {
      ...
      // Add a case for AppLifecycleState.hidden if necessary
      break;
  ...
  // Include all the UI building methods from the previous code
  Widget _buildInstructionsCard() {
    if (!_state.showInstructions || _isLoading) return SizedBox.shrink();
    ...
    return SlideTransition(
      ...
      child: AnimatedOpacity(
        ...
        child: Card(
          elevation: 8, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)), color: Colors.blue[50],
          child: Padding(padding: const EdgeInsets.all(16.0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              SizedBox(width: 8),
              Expanded(child: Text('How to Use Attendance System', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.blue[800]))),
              IconButton(icon: Icon(Icons.close, color: Colors.grey), onPressed: () => _updateState(_state.copyWith(showInstructions: false))),
            ]),
            ...
            SizedBox(height: 16),
            Container(
              padding: EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.amber[100], borderRadius: BorderRadius.circular(8)),
              child: Row(children: [
                Icon(Icons.gps_fixed, color: Colors.amber[700]), SizedBox(width: 8),
                Expanded(child: Text('Location tracked every 30 sec. Distance calculated automatically', style: TextStyle(color: Colors.amber[700], fontWeight: FontWeight.w500, fontSize: 12))),
              ]),
            ),
          ])),
        ),
  ...
  Widget _buildInstructionItem(IconData icon, String title, String description, Color color) {
    return Row(children: [
      Container(padding: EdgeInsets.all(8), decoration: BoxDecoration(color: color.withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: Icon(icon, color: color, size: 20)),
      SizedBox(width: 12),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(title, style: TextStyle(fontWeight: FontWeight.bold, color: color, fontSize: 12)),
        Text(description, style: TextStyle(color: Colors.grey[700], fontSize: 11)),
      ])),
    ]);
  }
  ...
  Widget _buildAttendanceCircle() {
    ...
    if (_state.isLoggedIn) {
      if (_state.isOnBreak) {
        Duration currentBreakDuration = _state.breakStartTime != null ? DateTime.now().difference(_state.breakStartTime!) : Duration.zero;
    ...
    Widget circle = Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: circleColor.withOpacity(0.3), spreadRadius: 8, blurRadius: 20, offset: const Offset(0, 5))],
      ),
      child: Center(child: Text(displayText, style: TextStyle(color: textColor, fontSize: 18, fontWeight: FontWeight.bold, height: 1.2), textAlign: TextAlign.center)),
    );
    ...
    return Column(children: [
      ...
      AnimatedSwitcher(
        ...
        child: Text(instructionText, key: ValueKey(instructionText), style: TextStyle(color: circleColor, fontSize: 16, fontWeight: FontWeight.w600), textAlign: TextAlign.center),
      ),
    ]);
  }
    ...
    return Column(children: [
      Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: Colors.grey.withOpacity(0.3), spreadRadius: 8, blurRadius: 20, offset: const Offset(0, 5))],
        ),
        child: Stack(alignment: Alignment.center, children: [
          CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(Colors.blue), strokeWidth: 3),
          Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.hourglass_empty, color: Colors.grey, size: 40), SizedBox(height: 8),
            Text('LOADING', style: TextStyle(color: Colors.grey, fontSize: 16, fontWeight: FontWeight.bold)),
          ]),
        ]),
      ),
      SizedBox(height: 16),
      Text('Initializing...', style: TextStyle(color: Colors.grey, fontSize: 16, fontWeight: FontWeight.w600), textAlign: TextAlign.center),
    ]);
  }
        ...
        child: Stack(
          ...
          children: [
            ...
            Column(
              ...
              children: [
                Icon(Icons.hourglass_empty, color: Colors.white, size: 40),
                SizedBox(height: 8),
                Text(
                  'WAITING',
                  style: TextStyle(
...
Future<void> _manualCheckApprovalStatus() async {
  ...
  try {
    ...
    print('❌ Manual status check failed: $e');
...
Future<ApprovalCheckResult> _performComprehensiveStatusCheck(String approvalId) async {
  try {
    ...
    print('⏰ Status check timeout: $e');
    rethrow;
  } on FirebaseException catch (e) {
    print('🔥 Firestore error: ${e.code} - ${e.message}');
    rethrow;
  } catch (e) {
    print('❌ Unexpected error in status check: $e');
    rethrow;
  }
}
...
void _updateApprovalWaitingUI(Map<String, dynamic> data) {
  ...
  try {
    ...
    print('❌ Error in updateApprovalWaitingUI: $e');
  ...
  return showDialog<void>(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        ...
        content: Text('Are you sure you want to cancel this login approval request?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: Text('No, Keep Waiting'),
          ),
          ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.red,
...
void _cleanupApprovalListeners() {
  ...
  Widget _buildLogoutButton() {
    if (!_state.isLoggedIn || _state.isOnBreak || _state.remainingShiftTime.inSeconds <= 0 || _isLoading) return SizedBox.shrink();

    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16.0),
      child: SizedBox(width: double.infinity, child: ElevatedButton.icon(
        style: ElevatedButton.styleFrom(
          backgroundColor: Colors.red, foregroundColor: Colors.white, padding: EdgeInsets.symmetric(vertical: 16),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        ),
        icon: Icon(Icons.logout), onPressed: _showLogoutConfirmationDialog,
        label: Text('EARLY LOGOUT', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
      )),
    );
  }

  Widget _buildLocationBanner() {
    if (_locationEnabled || _isLoading) return SizedBox.shrink();

    return Container(
      width: double.infinity, padding: const EdgeInsets.all(12.0), margin: const EdgeInsets.all(16.0),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [Colors.orange[100]!, Colors.orange[50]!]), borderRadius: BorderRadius.circular(12.0), border: Border.all(color: Colors.orange),
      ),
      child: Row(children: [
        Icon(Icons.location_off, color: Colors.orange), SizedBox(width: 8),
        Expanded(child: Text('Enable location for accurate attendance tracking', style: TextStyle(color: Colors.orange[700], fontWeight: FontWeight.w500))),
        ElevatedButton(
          style: ElevatedButton.styleFrom(backgroundColor: Colors.orange, foregroundColor: Colors.white, minimumSize: Size(60, 30)),
          onPressed: _checkLocationPermission, child: Text('Enable', style: TextStyle(fontSize: 12)),
        ),
      ]),
    );
  }
  ...
  Widget _buildUserInfo() {
    ...
    return Card(
      ...
      child: Padding(padding: const EdgeInsets.all(16.0), child: Column(children: [
        Row(children: [
          CircleAvatar(backgroundColor: Colors.blue[100], radius: 25, child: Icon(Icons.person, color: Colors.blue, size: 30)),
          SizedBox(width: 16),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(_state.userName, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            Text(_state.isLoggedIn ? "Currently Working" : "Not Logged In", style: TextStyle(color: _state.isLoggedIn ? Colors.green : Colors.grey, fontSize: 14)),
            Text('Employee ID: ${_state.empId}', style: TextStyle(color: Colors.grey[600], fontSize: 12)),
            Text(DateFormat('dd MMM yyyy - EEEE').format(DateTime.now()), style: TextStyle(color: Colors.grey[600], fontSize: 12)),
          ])),
          Container(
            padding: EdgeInsets.symmetric(horizontal: 12, vertical: 6),
            decoration: BoxDecoration(color: _state.isLoggedIn ? Colors.green[100] : Colors.grey[200], borderRadius: BorderRadius.circular(20)),
            child: Text(_state.isLoggedIn ? "ACTIVE" : "OFFLINE", style: TextStyle(color: _state.isLoggedIn ? Colors.green[700] : Colors.grey[600], fontWeight: FontWeight.bold, fontSize: 12)),
          ),
        ]),
        if (_state.isLoggedIn) ...[
          Divider(height: 24),
  ...
  Widget _buildInfoItem(IconData icon, String title, String value) {
    return Column(children: [
      Icon(icon, color: Colors.blue, size: 20), SizedBox(height: 4),
      Text(title, style: TextStyle(fontSize: 12, color: Colors.grey[600])), SizedBox(height: 2),
  ...
  Widget _buildLoadingUserInfo() {
    return Card(
      ...
      child: Padding(padding: const EdgeInsets.all(16.0), child: Row(children: [
        CircleAvatar(backgroundColor: Colors.grey[300], radius: 25, child: CircularProgressIndicator(strokeWidth: 3, valueColor: AlwaysStoppedAnimation<Color>(Colors.blue))),
        SizedBox(width: 16),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(width: 150, height: 20, decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(4))),
          SizedBox(height: 8), Container(width: 100, height: 16, decoration: BoxDecoration(color: Colors.grey[200], borderRadius: BorderRadius.circular(4))),
          SizedBox(height: 4), Container(width: 120, height: 14, decoration: BoxDecoration(color: Colors.grey[200], borderRadius: BorderRadius.circular(4))),
        ])),
        Container(
          padding: EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: Colors.grey[200], borderRadius: BorderRadius.circular(20)),
          child: Text("LOADING", style: TextStyle(color: Colors.grey[600], fontWeight: FontWeight.bold, fontSize: 12)),
        ),
      ])),
    );
  }
  ...
  Widget _buildErrorUserInfo() {
    return Card(
      ...
      child: Padding(padding: const EdgeInsets.all(16.0), child: Column(children: [
        Row(children: [
          CircleAvatar(backgroundColor: Colors.red[100], radius: 25, child: Icon(Icons.error_outline, color: Colors.red, size: 30)),
          SizedBox(width: 16),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text("Error Loading User", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.red)),
            Text(_errorMessage ?? 'Unknown error occurred', style: TextStyle(color: Colors.grey[600], fontSize: 12)),
          ])),
        ]),
        SizedBox(height: 12),
        ElevatedButton.icon(onPressed: _initializeUserData, icon: Icon(Icons.refresh), label: Text('Retry Loading'), style: ElevatedButton.styleFrom(backgroundColor: Colors.blue, foregroundColor: Colors.white)),
      ])),
    );
  }
  ...
  Widget _buildErrorWidget() {
    return Card(
      elevation: 4, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
      child: Padding(padding: const EdgeInsets.all(16.0), child: Column(children: [
        Icon(Icons.error_outline, size: 64, color: Colors.red), SizedBox(height: 16),
        Text('Something went wrong', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.red)),
        SizedBox(height: 8), Text(_errorMessage ?? 'Unknown error occurred', style: TextStyle(color: Colors.grey[600]), textAlign: TextAlign.center),
        SizedBox(height: 16),
        ElevatedButton.icon(onPressed: _initializeApp, icon: Icon(Icons.refresh), label: Text('Retry'), style: ElevatedButton.styleFrom(backgroundColor: Colors.blue, foregroundColor: Colors.white)),
      ])),
    );
  }

  // Essential UI methods to include:
  ...
  Future<void> _showLogoutConfirmationDialog() async {
    // Implementation of logout confirmation dialog
  }
